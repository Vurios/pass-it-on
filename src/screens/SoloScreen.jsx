    ...
    <GameLayout timer={<QuestionTimer state={state} onComplete={onTimeExpired} onSecondChange={onSecondChange} />}>
      <div className="flex h-full flex-col justify-between py-1">
        ...
        <div className="my-auto flex flex-col gap-3 sm:gap-4">
          <div className="rounded-[16px] border-chunky border-ink bg-white p-3 text-center shadow-hard sm:p-4">
            <p className="font-display text-xs font-bold uppercase tracking-[0.08em] text-ocean sm:text-sm">The Event</p>
            <h1 className="safe-copy mt-1 font-display text-lg font-bold leading-tight sm:text-xl lg:text-2xl host:text-4xl">
              {item.material.event}
            </h1>
          </div>
          ...
          <div className="grid gap-3 sm:gap-4 md:grid-cols-2">
            {item.material.sources.map((source, index) => (
              <button
                ...
                className={`${sourceColours[index]} press flex flex-col justify-between rounded-[16px] border-chunky border-ink p-3.5 text-left shadow-hard focus-visible:outline-4 focus-visible:outline-offset-4 focus-visible:outline-ink sm:p-4`}
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-display text-xl font-bold sm:text-2xl host:text-4xl">{source.label}</span>
                ...
                <span className="safe-copy mt-1.5 block font-display text-sm font-semibold leading-snug sm:text-base host:text-2xl">
                  {source.headline}
                </span>
              </button>
            ))}
    ...
    <GameLayout>
      <div className="flex h-full flex-col justify-between py-1">
        ...
        <div className="my-auto flex flex-col gap-3 sm:gap-4">
            ...
            {item.material.sources.map((source, index) => {
              ...
              return (
                <Card
                  key={source.id}
                  fill="white"
                  className={`${sourceRevealColours[index]} ${correct ? 'ring-4 ring-sunshine shadow-hard !opacity-100' : '!opacity-75'} flex flex-col justify-between p-3.5 sm:p-4`}
                >
                  <div>
                    <div className="flex items-center justify-between">
                      <p className="safe-copy font-display text-base font-bold sm:text-lg host:text-2xl">{source.label}: {source.source}</p>
                    ...
                    <p className="safe-copy mt-1.5 font-body text-xs font-semibold leading-snug sm:text-sm host:text-xl">{source.headline}</p>
                  </div>
                </Card>
              )
            })}
          ...
          <Card className="reveal-banner bg-paper p-3.5 sm:p-4" tilt="right">
            <div className="flex flex-wrap items-center gap-2 sm:gap-3">
              <SealCheck size={28} weight="fill" className="text-ocean" aria-hidden="true" />
              <h2 className="safe-copy font-display text-lg font-bold sm:text-xl host:text-3xl">{answer?.source}: {item.technique}</h2>
              {item.fabricated && <FabricatedStamp />}
            </div>
            <p className="safe-copy mt-1.5 font-body text-xs font-medium leading-relaxed sm:text-sm host:text-2xl">{item.explanation}</p>
          </Card>
  ...
  return (
    <GameLayout timer={<QuestionTimer state={state} onComplete={onTimeExpired} onSecondChange={onSecondChange} />}>
      <div className="flex h-full flex-col justify-between py-1">
        ...
        <div className="my-auto mx-auto flex w-full max-w-4xl flex-col items-center justify-center gap-5 sm:gap-6">
          <div className="w-full rounded-[20px] border-chunky border-ink bg-white p-5 text-center shadow-hard sm:p-7">
          ...
          <BigButton
            variant="sunshine"
            className="!h-13 sm:!h-15 w-full max-w-md text-lg sm:text-xl font-bold text-ink shadow-hard host:text-3xl"
  ...
  return (
    <GameLayout>
      <div className="flex h-full flex-col justify-between py-1">
        ...
        <div className="my-auto flex flex-col gap-3 sm:gap-4">
          <div className="rounded-[20px] border-chunky border-ink bg-white p-4 shadow-hard sm:p-6">
            ...
            <div className="mt-3 grid gap-2.5 sm:grid-cols-2">
              {item.material.phrases.map((phrase, index) => {
                ...
                return (
                  <div
                    key={phrase}
                    className={`${isCorrectPhrase ? 'bg-coral text-white' : isWrongFlag ? 'bg-paper border-coral' : 'bg-cream text-ink'} rounded-[14px] border-chunky border-ink p-3 sm:p-3.5`}
                  >
                    <div className="flex items-start gap-2">
                      {isCorrectPhrase && <Check size={22} weight="bold" aria-label="Manipulative phrase" className="shrink-0" />}
                      {isWrongFlag && <X size={22} weight="bold" aria-label="Incorrect flag" className="shrink-0 text-coral" />}
                      <p className="safe-copy font-display text-sm font-bold sm:text-base host:text-2xl">{phrase}</p>
                    </div>
                    {isCorrectPhrase && <p className="safe-copy mt-1 font-body text-xs font-bold opacity-90 sm:text-sm host:text-xl">{item.technique}</p>}
                    {isWrongFlag && <p className="safe-copy mt-1 font-body text-xs font-bold text-coral sm:text-sm">Not a manipulation signal.</p>}
                  </div>
                )
              })}
            </div>
          ...
          <Card className="reveal-banner bg-paper p-3.5 sm:p-4" tilt="left">
            <div className="flex flex-wrap items-center gap-2">
              <h2 className="safe-copy font-display text-lg font-bold sm:text-xl host:text-3xl">{item.technique}</h2>
              {item.fabricated && <FabricatedStamp />}
            </div>
            <p className="safe-copy mt-1.5 font-body text-xs font-medium leading-relaxed sm:text-sm host:text-2xl">{item.explanation}</p>
          </Card>
      ...
      <div className="flex h-full flex-col justify-between py-1">
        ...
        <div className="my-auto flex flex-col gap-3 sm:gap-4">
          <div className="rounded-[16px] border-chunky border-ink bg-white p-3 text-center shadow-hard sm:p-4">
            ...
            <h1 className="safe-copy mt-1 font-display text-lg font-bold leading-tight sm:text-xl host:text-3xl">{item.material.claim}</h1>
          </div>
            ...
            {item.material.retellings.map((retelling, index) => {
              ...
              return (
                <button
                  ...
                  className={`${sourceColours[index]} ${chosen ? 'press-held' : 'press shadow-hard'} flex flex-col justify-between rounded-[16px] border-chunky border-ink p-3.5 text-left focus-visible:outline-4 focus-visible:outline-offset-4 focus-visible:outline-ink sm:p-4`}
                >
                  <div className="flex items-center justify-between">
                    <span className="font-display text-xl font-bold host:text-3xl">{retelling.label}</span>
                    <span className={`${chosen ? 'bg-paper text-ink' : 'bg-cream text-ink opacity-60'} grid h-8 w-8 shrink-0 place-items-center rounded-full border-2 border-ink font-display text-sm font-bold`}>
                      {chosen ? position + 1 : '?'}
                    </span>
                  </div>
                  <span className="safe-copy mt-1.5 block font-display text-sm font-semibold leading-snug sm:text-base host:text-xl">{retelling.text}</span>
                </button>
              )
            })}
          </div>

          <div className="mx-auto grid w-full max-w-xl gap-3 sm:grid-cols-[1fr_auto]">
            <BigButton
              variant="sunshine"
              className="!h-12 !min-h-0 text-base font-bold text-ink shadow-hard sm:!h-13 sm:text-lg host:text-3xl"
              disabled={!full}
            ...
            <BigButton variant="ocean" className="!h-12 !min-h-0 gap-1.5 px-4 text-xs font-bold text-white sm:!h-13 sm:w-auto sm:text-sm host:text-3xl" disabled={selections.length === 0} onClick={() => dispatch({ type: 'RESET_CHAIN' })}>
            ...
            {ordered.map((retelling, index) => (
              <li key={retelling.id}>
                <Card fill="white" tilt={index % 2 ? 'right' : 'left'} className={`${index === 0 ? '!bg-lime' : index === ordered.length - 1 ? '!bg-coral !text-white' : ''} flex h-full flex-col justify-between p-3.5`}>
                  <p className="safe-copy font-display text-sm font-bold sm:text-base host:text-2xl">
                    <span className="opacity-60">{index + 1}. </span>{retelling.text}
                  </p>
                  <p className="safe-copy mt-1 font-body text-xs font-semibold host:text-xl">{retelling.note}</p>
                </Card>
              </li>
            ))}
          </ol>
  ...
  return (
    <GameLayout>
      <div className="flex h-full flex-col justify-between py-1">
        ...
        <div className="my-auto mx-auto flex w-full max-w-3xl flex-col items-center justify-center gap-4 sm:gap-5">
          <Card fill="white" tilt="left" className="w-full p-5 text-center sm:p-7 shadow-hard">
            <Star className="mx-auto text-sunshine" size={48} weight="fill" aria-hidden="true" />
            <p className="mt-1 font-body text-sm font-bold host:text-2xl">Room Score</p>
            <h1 className="mt-1 font-display text-4xl font-bold leading-none sm:text-5xl">{state.player.score}</h1>
            <p className="mt-2 font-display text-xl font-bold text-ocean sm:text-2xl host:text-3xl">{title}</p>
          ...
          <div className="grid w-full max-w-xl gap-3 sm:grid-cols-2">
            <BigButton variant="sunshine" className="!h-12 !min-h-0 text-base font-bold text-ink shadow-hard sm:!h-13 sm:text-lg host:text-3xl" onClick={onRecap}>
              See What You Learned
            </BigButton>
            <BigButton variant="coral" className="!h-12 !min-h-0 text-base font-bold text-white shadow-hard sm:!h-13 sm:text-lg host:text-3xl" onClick={onPlayAgain}>
              Play Again
            </BigButton>
          </div>
        </div>
